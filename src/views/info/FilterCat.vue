<template>
  ...
  >
    ...
    <ul>
      <li v-for="prod in filterProductsByRange" :key="prod">
        Product Name : {{ prod.name }} - Price : {{ prod.price }}
        {{ prod.category }} Skill: {{ prod.skill }}
      </li>
    </ul>

    <!-- FILTER BY Name -->
    <div v-if="switcher" class="flex flex-col items-center justify-center">
      <h3 class="font-extrabold text-teal-500">Filter By Name</h3>
      <input type="text" v-model="name" placeholder="Filter By Name" />
      <ul>
        <li v-for="prod in filterProductsByName" :key="prod">
          Product Name : {{ prod.name }} - Price : {{ prod.price }}
          {{ prod.category }} Skill: {{ prod.skill }}
        </li>
      </ul>
    ...
    <div v-if="!switcher" class="flex flex-col items-center justify-center">
      ...
      <ul>
        <li v-for="prod in filterProductsBySkill" :key="prod">
          Product Name : {{ prod.name }} - Price : {{ prod.price }}
          {{ prod.category }} Skill: {{ prod.skill }}
        </li>
      </ul>
    </div>
    <button class="btn btn-accent" @click="switcher = !switcher">
      Skill/name
    </button>
    <!-- FILTER BY Category -->
    ...
    <ul>
      <li v-for="product in filterProductsByCategory" :key="product">
    ...
    <button
      @click="$router.push('SubmitForm')"
      class="btn btn-primary"
    ></button>
    <button @click="$router.push('ToDo')" class="btn btn-primary">Todo</button>
  </div>
</template>
<script setup>
import { ref } from "vue";
...
const filterProductsByRange = computed(() => {
  return products.value.filter((product) =>
    product.price > 0 && product.price < range.value ? product : ""
  );
});
// FILTER BY SKILL
const filterProductsBySkill = computed(() => {
  return products.value.filter((product) => {
    return product.skill.toLowerCase().indexOf(skill.value.toLowerCase()) != -1;
  });
});
// // FILTER BY NAME SEARCH
const filterProductsByName = computed(() => {
  return products.value.filter((product) => {
    return product.name.toLowerCase().indexOf(name.value.toLowerCase()) != -1;
  });
});
// FILTER BY OPTION IN SELECTOR
const filterProductsByCategory = computed(() => {
  return products.value.filter(
    (product) => !product.category.indexOf(category.value)
  );
});
</script>
